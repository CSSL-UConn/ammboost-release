...
func (bz *ChainBoost) updateMainChainBCTransactionQueueCollect() {
	...
	PorTxSize, ServAgrProposeTxSize, PayTxSize, StoragePayTxSize = blockchain.TransactionMeasurement(bz.SectorNumber, bz.SimulationSeed)
	ServAgrCommitTxSize = 0

	var numOfPoRTxsMC, numOfPoRTxsSC, numOfServAgrProposeTxs, numOfStoragePaymentTxs, numOfServAgrCommitTxs, numOfRegularPaymentTxs int
	...
	for i := range rowsMarketMatching {
		...
		if !rowsMarketMatching[i].Published && !rowsMarketMatching[i].TxIssued {
			...
			// --- Add TxPor
			tx := blockchain.MainChainFirstQueueEntry{Name: "TxPor", Size: PorTxSize, Time: time.Now(), RoundIssued: int(bz.MCRoundNumber.Load()), ServAgrId: i + 1}
			mcFirstQueueTxs = append(mcFirstQueueTxs, tx)
			numOfPoRTxsMC++
			//-------------------------------------------------------------------
...
func (bz *ChainBoost) StoragePaymentMainChainBCTransactionQueueCollect() error {
	...
	var err error
	_, _, _, StoragePayTxSize := blockchain.TransactionMeasurement(bz.SectorNumber, bz.SimulationSeed)
	mcFirstQueueTxs := make([]blockchain.MainChainFirstQueueEntry, 0)
