...
func Simulate(configurations []Config,
	...
	if rootSim != nil {
		for run, configuration := range configurations {
			...
			ChainBoostProtocol.DailyVolume = configuration.DailyVolume
			ChainBoostProtocol.BlockTime = configuration.BlockTime
			ChainBoostProtocol.SwapRate = configuration.SwapRate
			ChainBoostProtocol.MintRate = configuration.MintRate
			ChainBoostProtocol.CollectRate = configuration.CollectRate
			ChainBoostProtocol.BurnRate = configuration.BurnRate
			ChainBoostProtocol.UsersNbr = configuration.UsersNbr
			ChainBoostProtocol.DepositMode = configuration.DepositMode
			...
			ChainBoostProtocol.StopDeposit.Store(false)
			// ----
			...
			ChainBoostProtocol.Done = <-ChainBoostProtocol.DoneRootNode
			log.LLvl1(rootSC.Server.ServerIdentity.Address, ": Final result is", ChainBoostProtocol.Done)
...
func NewChainBoostProtocol(n *onet.TreeNodeInstance) (onet.ProtocolInstance, error) {
	...
	bz := &MainAndSideChain.ChainBoost{
		...
		StopDeposit:         *atomic.NewBool(false),
	}
		...
		if child != ChainBoostProtocol.TreeNode() {
			...
			for time.Since(start) < timeout {
				err := ChainBoostProtocol.SendTo(child, &MainAndSideChain.HelloChainBoost{
					...
					// =------------ AMM Boost fields:
					DailyVolume: ChainBoostProtocol.DailyVolume,
					BlockTime:   ChainBoostProtocol.BlockTime,
					SwapRate:    ChainBoostProtocol.SwapRate,
					BurnRate:    ChainBoostProtocol.BurnRate,
					CollectRate: ChainBoostProtocol.CollectRate,
					MintRate:    ChainBoostProtocol.MintRate,
					UsersNbr:    ChainBoostProtocol.UsersNbr,
				})
				if err != nil {
