...
func GetUsersDbAbsPath() string {
	return GetAbsPath("users.db")
}
func GetKeysDbAbsPath() string {
	return GetAbsPath("keys.db")
}

var mainchainpath string = GetMainChainAbsPath()
var sidechainpath string = GetSideChainAbsPath()
var userSidechainPath string = GetUsersDbAbsPath()
var keysDBAbsPath string = GetKeysDbAbsPath()
...
func PickleKeys(privKey []byte, pubKey []byte) error {
	deleteDbIfExists(keysDBAbsPath)
	var err error
	if keysDB == nil {
		keysDB, err = sql.Open("sqlite", keysDBAbsPath)
	}
	_, err = keysDB.Query("CREATE TABLE keys(privateKey BLOB, publicKey BLOB)")
	stmt := `INSERT INTO keys(privateKey, publicKey) VALUES (?,?)`
	valueArgs := make([]interface{}, 2)
	valueArgs[0] = privKey
	valueArgs[1] = pubKey
	_, err = keysDB.Exec(stmt, valueArgs...)
	return err

}

func UnpickleKeys() (privkey *big.Int, pubKey bgls.Point, err error) {

	if keysDB == nil {
		keysDB, err = sql.Open("sqlite", keysDBAbsPath)
	}
	stmt := "SELECT privateKey, publicKey from keys"
	rows, err := keysDB.Query(stmt)
	if err != nil {
		return nil, nil, err
	}
	defer rows.Close()
	for rows.Next() {
		var sk_bytes []byte
		var pk_bytes []byte
		if err := rows.Scan(&sk_bytes, &pk_bytes); err != nil {
			return nil, nil, err
		}
		curve := bgls.CurveSystem(bgls.Altbn128)
		pubKey, truth := curve.UnmarshalG2(pk_bytes)
		if !truth {
			panic("couldn't unpickle keys")
		}
		return new(big.Int).SetBytes(sk_bytes), pubKey, nil
	}
	return nil, nil, errors.New("not found")
}

func KeyDBMustExist() {
	if _, err := os.Stat(keysDBAbsPath); errors.Is(err, os.ErrNotExist) {
		panic(err)
	}
}

func InitializeUsersDbTables() error {
	deleteDbIfExists(userSidechainPath)
	var err error
	if userDB == nil {
		userDB, err = sql.Open("sqlite", userSidechainPath)
	}
	_, err = userDB.Query(`
		CREATE TABLE Users ("AmmId" text PRIMARY KEY, "privKey" BLOB )
	`)
	return err
}
func encode(privateKey *ecdsa.PrivateKey) []byte {
	privkey := crypto.FromECDSA(privateKey)

	return privkey
}

func decode(privkey []byte) *ecdsa.PrivateKey {
	privateKey := crypto.ToECDSAUnsafe(privkey)
	return privateKey
}

func UserDBMustExist() {
	if _, err := os.Stat(userSidechainPath); errors.Is(err, os.ErrNotExist) {
		panic(err)
	}
}

func BatchInsertIntoUsersTable(users []UserInfoLite) error {
	var err error
	if userDB == nil {
		userDB, err = sql.Open("sqlite", userSidechainPath)
	}
	...
	for _, row := range users {
		prkey := encode(row.PrivKey)
		valueStrings = append(valueStrings, "(?, ?)")
		valueArgs = append(valueArgs, row.AmmAddress)
		valueArgs = append(valueArgs, prkey)
	}
	stmt := fmt.Sprintf(`INSERT INTO Users(AmmId, privkey)
                                 VALUES %s`, strings.Join(valueStrings, ","))
	_, err = userDB.Exec(stmt, valueArgs...)
	return err
}

func GetUsers() ([]UserInfoLite, error) {
	var err error
	retval := make([]UserInfoLite, 0)
	if userDB == nil {
		userDB, err = sql.Open("sqlite", userSidechainPath)
	}
	stmt := "Select AmmId, privkey from Users LIMIT 100"
	rows, err := userDB.Query(stmt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	for rows.Next() {
		var amm string
		var pk []byte
		if err := rows.Scan(&amm, &pk); err != nil {
			return nil, err
		}
		privKey := decode(pk)
		item := UserInfoLite{AmmAddress: amm, PrivKey: privKey, Pubkey: &privKey.PublicKey}
