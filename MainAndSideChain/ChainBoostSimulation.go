...
var Pool *ammcore.LiquidityPool
...
func (eth *EthInfo) GetAddress() string {
	return crypto.PubkeyToAddress(*eth.pubkey).Hex()
}
...
type ChainBoost struct {
	...
	*onet.TreeNodeInstance
	...
	DepositArray []sync.Map
	TimeStart    time.Time
	DepositMode  int // 0 for random, 1 for once per epoch;

	privKey *big.Int
	pubKey  bgls.Point
	curve   bgls.CurveSystem

	StopDeposit   atomic.Bool
	StopDepositWg sync.WaitGroup
...
func (bz *ChainBoost) SelectRandomDepositor(roundnbr int) string {
	keys := make([]string, 0)
	bz.printDeposits(roundnbr)
	bz.DepositArray[roundnbr].Range(func(key any, value any) bool {
		realkey := key.(string)
		//		fmt.Println("User: ", key, "BalanceA: ", Users[realkey].Balance.TokenA, "BalanceB: ", Users[realkey].Balance.TokenB)
		if Users[realkey].Balance.TokenA.Cmp(lib.Exponentiate(1, 18)) > 0 && Users[realkey].Balance.TokenB.Cmp(lib.Exponentiate(1, 18)) > 0 {
			keys = append(keys, key.(string))
		}
		return true
	})

	rnd := rand.Intn(len(keys))
	return keys[rnd]
}
...
func (bz *ChainBoost) Dispatch() error {
	...
	for running {
		...
		case msg := <-bz.HelloChan:
			...
			bz.DailyVolume = msg.DailyVolume
			bz.BlockTime = msg.BlockTime
			bz.SwapRate = msg.SwapRate
			bz.BurnRate = msg.BurnRate
			bz.CollectRate = msg.CollectRate
			bz.MintRate = msg.MintRate
			bz.DepositMode = msg.DepositMode
			bz.StopDeposit = *atomic.NewBool(false)
